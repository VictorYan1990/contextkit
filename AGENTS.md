...
- **Rules.** Short and always-on. Central ships only `rules/USER.template.md` —
  a blank profile structure — never a filled-in one, because this repository is
  public. A real profile belongs in a private module's `rules/USER.md`. Skills,
  personas, and knowledge stay person-neutral.
